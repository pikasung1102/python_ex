# 369게임 만들기
'''
친구들끼리 믾이 하는 369게임을 만들어 봅시다
1부터 99까지 1씩 증가하면서 숫자에 3, 6, 9가 들어 있을 때마다 숫자와 함께 짝을 출력합니다.
6 -> 짝
33 -> 짝! 짝!
99 -> 짝! 짝!
'''
...
for num in range(1, 100):
    ...
    # quiz) 열차 교대 시긴 알아내기
    '''
    대전역에는 3개 노선의 열차가 오전 9시 부터 오후 6시까지 교차 운행한다
    3대의 열차가 교차하는 시간을 구해 열차 충돌 사고를 막으세요
    (단 매일 오전 9시에 대전역 에서 모든 열차가 출발한다 )
    -----------------------------------
...
for n in range(1, 541):
    if n % trainA == 0 and n % trainB == 0 and n % trainC == 0:
        ...
        # print(n % 60, end='')           # 분
        # print('분')
        magStr = '00' if n % 60 == 0 else str (n % 60)
        print(f'{9 + n // 60}시 {magStr} 분')
...
팩토리얼(factorial, !) n! 1부터 양의 정수 n까지의 모든 정수를 합친 값을 말한다.
