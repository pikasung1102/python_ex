...
# 함수 정의하기
'''
사용자 함수를 만든다는 것을 '함수로 정의한다.'라고 합니다.
함수를 정의할 때 def 키워드를 사용합니다. 그리고 함수명을 콜론(:), 실행부를 이용ㅇ합니다.
'''
'''
def 함수명():
    실행부(함수 기능)
...
5. 두개 이상의 단어가 조합되는 경우 스네이크 또는 카멜 표기법을 사용하자.
    sendMssage():   calculateDistance():
'''
...
def startTempratureSensor():
    print('온도센서 작동을 시작합니다.')

def stopTempratureSensor():
    print('온도센서 작동을 중지합니다.')

# 함수 호출
startTempratureSensor()
stopTempratureSensor()

# quiz: 내 노트북은 몇 인치일까?
'''
고등학교 졸업 기념으로 노트북을 하나 장만햇습니다
...
# 길동이는 등산한 시간과 속도를 입력하면 이동한 거리를 계산해주는 프로그램을
# 함수를 이용하여 만들어봅시다.
# '''
...
# '''
# 출신 구가를 선택하면 해당하는 국가의 인사말이 출력되는 프로그램을
# 함수를 이용해서 만들어 봅시다
# 1.한국      2.USA       3.Japan
# '''
...
# quiz) 계산기 프로그램 by 함수
'''
사용ㅇ자가 숫자 2개를 입력하고 연산자를 선택하면 연산 결과가 출력되는 프로그램
을 함수를 이용해서 만들어봅시다.
'''
